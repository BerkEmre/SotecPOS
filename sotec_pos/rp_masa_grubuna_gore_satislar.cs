    ...
    public partial class rp_masa_grubuna_gore_satislar : DevExpress.XtraReports.UI.XtraReport
    {
        public rp_masa_grubuna_gore_satislar(DateTime tarih1, DateTime tarih2)
        {
            ...
            DataTable dt = SQL.get(" SELECT " +
                                   "     mk.masa_kategori, " +
                                   "     tutar = ISNULL((SELECT SUM(fh.miktar) FROM finans_hareket fh INNER JOIN adisyon a ON a.adisyon_id = fh.referans_id INNER JOIN masalar m ON m.masa_id = a.masa_id AND m.masa_kategori_id = mk.masa_kategori_id WHERE fh.silindi = 0 AND fh.hareket_tipi_parametre_id IN (25, 26, 27) AND fh.kayit_tarihi BETWEEN '" + tarih1.ToString("yyyy-MM-dd HH:mm:00.000") + "' AND DATEADD(DAY, 1, '" + tarih2.ToString("yyyy-MM-dd HH:mm:00.000") + "')), 0) " +
