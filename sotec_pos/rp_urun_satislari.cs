    ...
    public partial class rp_urun_satislari : DevExpress.XtraReports.UI.XtraReport
    {
        public rp_urun_satislari(DateTime tarih1, DateTime tarih2)
        {
            ...
            DataTable dt = SQL.get(" SELECT " +
                                   "     u.urun_adi, " +
                                   "     tutar = ISNULL((SELECT SUM(ak.miktar) * u.fiyat FROM adisyon_kalem ak WHERE ak.silindi = 0 AND ak.urun_id = u.urun_id AND ak.kayit_tarihi BETWEEN '" + tarih1.ToString("yyyy-MM-dd HH:mm:00.000") + "' AND DATEADD(DAY, 1, '" + tarih2.ToString("yyyy-MM-dd HH:mm:00.000") + "')), 0) " +
                                   " FROM urunler u WHERE u.silindi = 0 AND u.menu_aktif = 1 AND 0 != ISNULL((SELECT SUM(ak.miktar) * u.fiyat FROM adisyon_kalem ak WHERE ak.silindi = 0 AND ak.urun_id = u.urun_id AND ak.kayit_tarihi BETWEEN '" + tarih1.ToString("yyyy-MM-dd HH:mm:00.000") + "' AND DATEADD(DAY, 1, '" + tarih2.ToString("yyyy-MM-dd HH:mm:00.000") + "')), 0) ");
